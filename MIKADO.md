...
| X5 | Split margin bars (shared vs non-cancelling) + gap margin beside the verdict | `src/ui/model.ts`, `src/ui/App.tsx`, `src/ui/app.css` | Score cells show ghost band + solid core; core == band when unpaired and collapses when paired; gap ± renders under the verdict badge | Vitest on exact split identity + manual toggle check | low | S | done — PR #14 |
