...
| Y6 | Example tip inserts the URL without loading; fetch progress moves onto the Load button as a fixed-width spinner; success message dropped | `src/ui/Toolbelt.tsx`, `src/ui/app.css` | Tip only fills the box; button swaps label for spinner without changing width; no "Fetching…"/success lines | manual browser check of insert, mid-flight spinner and completion (§4: presentation-only) | low | S | done — PR #25 |
| Y7 | Fix ntp "what's this?" toggling the checkbox instead of opening its popover; popover citations link to the paper | `src/ui/PowerPanel.tsx`, `src/ui/popover.tsx`, `src/ui/app.css` | Trigger opens the popover and leaves the checkbox alone; every citation is a link to arXiv:2411.00640 | manual browser check of both popovers + checkbox still toggling (§4: presentation-only) | low | S | done — PR #26 |
| Y8 | Compress share-link CSV payloads (lz-string) and resize the example file so a loaded view is shareable | `src/data/share.ts`, `public/datasets/example-eval.csv`, `scripts/make_example_csv.py` | Example loads and "Copy link" succeeds; link restores in a fresh load; oversized-even-compressed CSVs still refused | Vitest (compression, guard with high-entropy fixture, example stays shareable) + manual round trip | low | M | done — PR #27 |
