...
`public/datasets/example-eval.csv` is the demo file for the "bring your own
data" path — synthetic, ~21 KB, generated by `python3 scripts/make_example_csv.py`
(`--check` re-verifies its statistics without rewriting it). It is small enough
to download and re-upload instantly, and small enough that a loaded view still
fits in a share link; it works either way:

    https://raw.githubusercontent.com/jv8-alt/fluke/main/public/datasets/example-eval.csv

Two fictional models, `baseline-v1` and `tuned-v2`, over two benchmarks whose
structure differs. Both show tuned-v2 ahead by almost exactly the same amount,
and only one of those leads is real: `reading` (150 questions in 30 passages)
leads by +13.3pp, which clears the naive margin but evaporates once passages
are counted once, while `arithmetic` (190 independent questions) leads by
+13.7pp and survives every correction. Same headline number, opposite
verdicts — the size of a gap says nothing about whether it is real. Asserted
in `src/data/example.test.ts`, which also pins that it stays shareable.
