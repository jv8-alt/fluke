...
data" path — synthetic, ~74 KB, generated by `python3 scripts/make_example_csv.py`
(`--check` re-verifies its statistics without rewriting it). It is small enough
to download and re-upload instantly, and works either way:
...
structure differs: `reading` (480 questions in 40 passages) shows a +8.3pp lead
that clears the naive margin but evaporates once passages are counted once,
while `arithmetic` (500 independent questions) shows a smaller +6.8pp lead that
survives every correction. The bigger gap is the fake one. Asserted in
